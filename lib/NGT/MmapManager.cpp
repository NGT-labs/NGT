...
void MmapManager::dumpHeap() const { _impl->dumpHeap(); }

bool MmapManager::isOpen() const { return _impl->isOpen; }

void *MmapManager::getEntryHook() const { return getAbsAddr(_impl->mmapCntlHead->entry_p); }

void MmapManager::setEntryHook(const void *entry_p) { _impl->mmapCntlHead->entry_p = getRelAddr(entry_p); }
...
bool MmapManager::openMemory(const std::string &filePath) {
  try {
    ...
    for (uint64_t i = 0; i < _impl->mmapCntlHead->unit_num; i++) {
      ...
      errno        = 0;
      _impl->mmapDataAddr[i] =
          mmap(NULL, _impl->mmapCntlHead->base_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset);
...
void MmapManager::free(const off_t p) {
  ...
  case REUSE_DATA_CLASSIFY: _impl->free_data_classify(p); break;
  case REUSE_DATA_QUEUE: _impl->free_data_queue(p); break;
  case REUSE_DATA_QUEUE_PLUS: _impl->free_data_queue_plus(p); break;
  default: _impl->free_data_classify(p); break;
  }
}
...
off_t MmapManager::reuse(const size_t size, reuse_state_t &reuse_state) {
  ...
  switch (_impl->mmapCntlHead->reuse_type) {
  case REUSE_DATA_CLASSIFY: ret_off = _impl->reuse_data_classify(size, reuse_state); break;
  case REUSE_DATA_QUEUE: ret_off = _impl->reuse_data_queue(size, reuse_state); break;
  case REUSE_DATA_QUEUE_PLUS: ret_off = _impl->reuse_data_queue_plus(size, reuse_state); break;
  default: ret_off = _impl->reuse_data_classify(size, reuse_state); break;
  }
...
uint16_t MmapManager::getUnitNum() const { return _impl->mmapCntlHead->unit_num; }
